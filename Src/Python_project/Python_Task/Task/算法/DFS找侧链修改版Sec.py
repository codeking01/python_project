        ...
        if len(side_set_list[flag_item[0]]) == 1:
            ...
            for single_atom_index in range(flag_item[1], flag_item[0] - 1, -1):
                ...
                if side_set_list[flag_item[0]] == side_set_list[flag_item[1]]:
                    ...
                    graph, erased_dic, main_list, main_flag = find_main_atoms(current_item_list=current_item_list,
                                                                              graph=graph, main_list=main_list,
                                                                              erased_dic=erased_dic)
                    if not main_flag:
                        ...
                        if flag_item != flag_index[-1]:
                            ...
                            # 找上一级侧链连接原子
                            if find_aim_atom(graph_item_list=graph[current_item_list[0]],
                                             aim_list=rest_side_atoms) != '':
                                aim_atom = find_aim_atom(graph_item_list=graph[current_item_list[0]],
                                                         aim_list=rest_side_atoms)
                                del_side_connections(graph=graph, current_item_list=current_item_list,
                                                     aim_atom=aim_atom)
                                # 将当前原子并到侧链链上去
                                side_set_list = find_insert_side_index(side_set_list=side_set_list, aim_atom=aim_atom,
                                                                       insert_atom=f'''{str(current_item_list)}''')
                            elif find_aim_atom(graph_item_list=graph[current_item_list[-1]],
                                               aim_list=rest_side_atoms) != '':
                                current_item_list.reverse()
                                aim_atom = find_aim_atom(graph_item_list=graph[current_item_list[0]],
                                                         aim_list=rest_side_atoms)
                        ...
                        # 单集合且是最后一个元素，那么肯定是并到主链上
                            ...
                            graph, erased_dic, main_list, main_flag = find_main_atoms(
                                current_item_list=current_item_list, graph=graph, main_list=main_list,
                                erased_dic=erased_dic)
            print('\nside_set_list.....\n', side_set_list)
            # todo 将等长的侧链集合单独处理，
    ...
    while item_index < len(side_set_list):
        ...
        if item_index != len(side_set_list) - 1:
            ...
            # ③ 删除该链路剩下的多余连接关系
                ...
                deal_multiple_atoms(graph=graph, main_list=main_list, side_set_list=side_set_list,
                                    flag_index=flag_index, erased_dic=erased_dic)
        else:
...
def gen_null_dict(no_circle_graph=None):
    for i in range(1, len(graph) + 1):
        no_circle_graph[str(i)] = []
    return no_circle_graph


def DFS(graph, start):
    # 记录断了环以后的连接关系,先将每个元素对应的列表建立出来
    no_circle_graph = gen_null_dict(no_circle_graph={})
    stack = []
    # 存储序列
    result = []
    # 存储已经访问过的节点
...
def get_all_max_road(graph=None):
    max_road = []
    for i in range(1, len(graph) + 1):
        result, no_circle_graph = DFS(graph=graph, start=str(i))
        ...
        if len(all_max_road) > len(max_road):
            ...
            max_mainList = copy.deepcopy(mainList)
            max_sideList = copy.deepcopy(sideList)
            result, no_circle_graph = copy.deepcopy(result), copy.deepcopy(no_circle_graph)
            # print('深度优先序列：', result)
            # print('\nNo_Circle_graph：', no_circle_graph)
            # print('*******************************************************')
    return max_road, max_final_result, max_mainList, max_sideList, result, no_circle_graph


if __name__ == "__main__":
    graph = exchange_graph(graph=graph)
    max_road, final_result,mainList, sideList, result, no_circle_graph = get_all_max_road(graph=graph)
